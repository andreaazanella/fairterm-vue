<template>
  <div class="language-section" :style="{ borderLeft: `3px solid ${badgeColor}` }">
    <!-- Header cliccabile: badge lingua, nome, conteggio termini, azioni, freccia espandi -->
    ...
    <!-- Corpo espanso: definizione, dettagli, e i termini di questa lingua -->
    <div v-if="expanded" class="language-section__body">
      ...
      <!-- External cross reference, Source, Notes -->
      ...
      <!-- Sezione termini: icona, titolo, bottone aggiungi -->
      ...
      <!-- Stato vuoto: nessun termine ancora aggiunto -->
      <p v-if="language.terms.length === 0" class="placeholder-note">No terms added yet.</p>

      <!-- Una card espandibile per ogni termine di questa lingua -->
      <TermCard
...
// Chiede conferma prima di emettere la richiesta di eliminazione: il
// messaggio menziona quanti termini verrebbero eliminati in cascata
